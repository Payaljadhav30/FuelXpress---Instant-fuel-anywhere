...
const getUserInfo = (id) => {
  return axios.get(API_URL + `user/getUserInfo/${id}`).then((response) => {
    return response;
  });
};
...
const fuelStationLogin = (email, password) => {
  return axios.post(API_URL + "fuel/login", {
    email,
    password,
  }).then((response) => {
...
const fuelStationRegister = (name, owner, email, password, phone, location) => {
  return axios.post(API_URL + "fuel/register", {
    name,
    owner,
    email,
    password,
...
const fuelInventoryUpdate = (quantity, stationId) => {
  return axios.put(API_URL + "fuel/updateFuel", {
    quantity,
    stationId,
  }).then((response) => {
    return response;
...
const getFuelStation = () => {
  return axios.get(API_URL + "fuel/getStations").then((response) => {
    return response;
  });
};

const getFuelStationByID = (id) => {
  return axios.get(API_URL + `fuel/${id}`).then((response) => {
    return response;
  });
};
...
const cancelOrder = (id) => {
  return axios.put(API_URL + `order/cancel`, { id }).then((response) => {
    return response;
  });
};

const acceptOrder = (id) => {
  return axios.put(API_URL + `order/accept`, { id }).then((response) => {
    return response;
  });
};

const deliveryOrder = (id) => {
  return axios.put(API_URL + `order/deliever`, { id }).then((response) => {
    return response;
  });
};

const getOrders = (id) => {
  return axios.get(API_URL + `order/getOrderByFuelStationId/${id}`).then((response) => {
    return response;
  });
};

const getUserOrders = (id) => {
  return axios.get(API_URL + `order/getOrderByUserId/${id}`).then((response) => {
    return response;
  });
};
...
const updateSellerProfilePassword = (stationId, password, newPassword) => {
  return axios.put(API_URL + `fuel/changePassword`, {
    stationId,
    password,
    newPassword,
  }).then((response) => {
...
const displayRazorpay = async (totalDeliveryCharge, setTransactionData) => {
  if (DEMO_MODE) {
    // ✅ Fake payment logic for demo
    setTimeout(() => {
      const fakeTransaction = {
        orderCreationId: "demo_order_123",
        razorpayPaymentId: "demo_payment_456",
        razorpayOrderId: "demo_order_123",
        razorpaySignature: "demo_signature_789",
      };
